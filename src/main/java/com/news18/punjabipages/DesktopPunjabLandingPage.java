...
/**
 * @author Abhishek Raj
 *This Class Contains elements and business logics for Punjab Desktop Landing  Page
 */
public class DesktopPunjabLandingPage {
	...
	public DesktopPunjabLandingPage(WebDriver driver) {
		this.driver = driver;
		PageFactory.initElements(driver, this);
	}

	/*Homepage First article Link  is present below the Line Two Menu bar in RHS*/
	@FindBy(xpath ="(//div[contains(@class,'topnews-right')]//ul//li//a[not(contains(@href,'live')) and contains(@href,'/news/')])[1]")
	...
	/*world link present at line one section*/
	@FindBy(xpath ="//div[contains(@class,'container')]//ul[contains(@class,'nav_bar')]//a[not(contains(@href,'/sports/')) and not(contains(@href,'/photogallery/')) and not(contains(@href,'/videos/'))]")
	...
	/**
	 * @author Sanjeeb
	 * This method is used to click on Section link present on line one section
	 */
